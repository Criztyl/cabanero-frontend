              ...
              <tbody>
                {students.length === 0 ? (
                  ...
                  <tr key={s.id} style={{ borderBottom:'1px solid rgba(255,255,255,0.04)',
                    ...
                    <td style={{ padding:'14px 16px', fontSize:'13px', color:'var(--secondary-color)', fontWeight:600 }}>
                      {s.student_id || '—'}
                    </td>
                    ...
                    <td style={{ padding:'14px 16px', fontSize:'13px', color:'var(--text-muted)' }}>{s.email}</td>
                    <td style={{ padding:'14px 16px' }}>
                      ...
                      {isAdmin() && (
                        <button onClick={() => handleDelete(s.id)} disabled={deleting === s.id}
                          style={{ background:'rgba(255,68,102,0.1)', border:'1px solid rgba(255,68,102,0.2)',
                                   ...
                                   color:'#ff4466', display:'flex', alignItems:'center', gap:'6px', fontSize:'12px' }}>
                          <Trash2 size={13} />
        ...
        {meta.lastPage > 1 && (
          <div style={{ display:'flex', justifyContent:'center', alignItems:'center',
            ...
            <button onClick={() => setPage(p => Math.max(1, p-1))} disabled={page===1}
              style={{ padding:'6px 14px', borderRadius:'8px', border:'1px solid var(--glass-border)',
                       background:'transparent', color: page===1 ? 'var(--text-muted)' : '#fff',
            ...
            <button onClick={() => setPage(p => Math.min(meta.lastPage, p+1))} disabled={page===meta.lastPage}
              style={{ padding:'6px 14px', borderRadius:'8px', border:'1px solid var(--glass-border)',
                       background:'transparent', color: page===meta.lastPage ? 'var(--text-muted)' : '#fff',
